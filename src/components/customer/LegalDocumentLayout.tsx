...
function LegalSectionCard({ section }: { section: LegalSection }) {
  ...
  return (
    <article
      id={slug}
      className="scroll-mt-28 rounded-2xl border border-[#004D55]/10 bg-white p-5 shadow-sm transition hover:border-[#FFC107]/45 hover:shadow-md sm:p-6"
    >
      <div className="flex items-start gap-3 sm:gap-4">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFC107] font-display text-sm font-extrabold text-[#004D55] sm:h-11 sm:w-11">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-4 space-y-3 text-sm leading-relaxed text-[#004D55]/80">
          ...
          {section.bullets && section.bullets.length > 0 && (
            <ul className="mt-4 space-y-2.5">
              {section.bullets.map((item) => (
                <li key={item} className="flex items-start gap-2.5 text-sm leading-relaxed text-[#004D55]/85">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#004D55]/10">
                    <Check className="h-3 w-3 text-[#004D55]" strokeWidth={2.5} />
                  </span>
                  <span>{item}</span>
                </li>
              ))}
                ...
                <div className="lg:sticky lg:top-24">
                  <AnimateIn animation="fade-up" delay={60}>
                    <div className="overflow-hidden rounded-2xl border border-[#004D55]/10 bg-white shadow-sm">
                      <div className="border-b border-[#FFC107]/25 bg-[#FFF8E1]/50 px-4 py-3">
                        <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[#004D55]">
                              ...
                              <li key={section.title}>
                                <a
                                  ...
                                  className={cn(
                                    ...
                                    isActive
                                      ? 'bg-[#FFC107]/15 font-bold text-[#004D55]'
                                      : 'text-[#004D55]/65 hover:bg-[#FFF8E1]/60 hover:text-[#004D55]',
                                  )}
                                >
                                  <span
                                    className={cn(
                                      'flex h-6 w-6 shrink-0 items-center justify-center rounded-lg font-display text-[10px] font-bold',
                                      isActive ? 'bg-[#FFC107] text-[#004D55]' : 'bg-[#004D55]/8 text-[#004D55]/60',
                                    )}
                                  >
                                    {sectionNumber(section.title)}
                                  </span>
              ...
              <div className="min-w-0 space-y-4 sm:space-y-5">
                ...
                {closingNote && (
                  <AnimateIn animation="fade-up" delay={80 + sections.length * 35}>
                    <div className="rounded-2xl border border-[#FFC107]/35 bg-[#FFF8E1]/60 p-5 sm:p-6">
                      <div className="flex items-start gap-3">
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#004D55]/10">
                          <Shield className="h-5 w-5 text-[#004D55]" />
                        </span>
                        <p className="text-sm leading-relaxed text-[#004D55]/85">{closingNote}</p>
                      </div>
                    </div>
                  </AnimateIn>
                )}
