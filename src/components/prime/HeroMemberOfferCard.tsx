...
export function HeroMemberOfferCard() {
  ...
  if (isLoggedIn) {
    ...
    return (
      <Link
        to="/account"
        className="group mt-3 inline-flex w-full max-w-xl items-center justify-between gap-3 rounded-full border border-[#FFC107]/45 bg-[#004D55]/50 px-4 py-2.5 backdrop-blur-md transition hover:border-[#FFC107]/70 hover:bg-[#004D55]/65 sm:w-auto sm:min-w-[320px]"
      >
        <span className="flex min-w-0 items-center gap-2.5">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#FFC107]">
            <Unlock className="h-4 w-4 text-[#004D55]" aria-hidden="true" />
          </span>
        ...
        <ArrowRight
          className="h-4 w-4 shrink-0 text-[#FFC107] transition group-hover:translate-x-0.5"
          aria-hidden="true"
        />
      </Link>
    )
    ...
    <div className="mt-3 w-full max-w-xl">
      <div className="flex items-center gap-2.5 rounded-full border border-[#FFC107]/40 bg-[#004D55]/55 px-2 py-2 backdrop-blur-md sm:gap-3 sm:px-2.5">
        ...
        <Link
          to="/login"
          state={{ from: '/' }}
          className="inline-flex shrink-0 items-center gap-1 rounded-full bg-[#FFC107] px-3.5 py-2 text-[10px] font-extrabold uppercase tracking-wide text-[#004D55] transition hover:bg-[#FFD54F] sm:px-4 sm:py-2.5 sm:text-[11px]"
        >
          Login
