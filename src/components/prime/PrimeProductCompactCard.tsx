  ...
  return (
    <article
      className={cn(
        ...
        inCart && 'ring-2 ring-[#FFC107] shadow-[0_4px_16px_rgba(255,193,7,0.2)]',
      )}
    >
      <div className="relative w-[5.75rem] shrink-0 overflow-hidden bg-slate-50 sm:w-32 lg:w-[38%] lg:min-h-[7.5rem]">
        ...
        {hasDiscount && (
          <span className="absolute left-1 top-1 z-10 rounded-md bg-[#E65100] px-1 py-0.5 text-[8px] font-extrabold text-white shadow-sm sm:left-2 sm:top-2 sm:px-1.5 sm:text-[9px] lg:hidden">
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-1.5 p-2.5 sm:p-3 lg:gap-2">
        <div className="min-w-0">
          ...
          {(brand || product.pieces != null) && (
            <div className="mt-1 flex min-w-0 items-center gap-1 sm:gap-1.5">
              {brand && (
                <span className="min-w-0 truncate border-y border-[#FFC107]/80 px-0.5 text-[9px] font-bold text-[#004D55] sm:border-none sm:px-0 sm:text-[10px] sm:font-semibold sm:text-[#004D55]/70">
                  {brand}
                </span>
              )}
              {product.pieces != null && (
                <span className="shrink-0 rounded bg-[#004D55] px-1 py-px text-[8px] font-bold text-[#FFC107] sm:bg-[#FFF8E1] sm:px-1.5 sm:text-[9px] sm:text-[#004D55] sm:ring-1 sm:ring-[#FFC107]">
                  {product.pieces} pcs
                </span>
              )}
            </div>
          ...
          <p className="mt-1.5 text-[12px] leading-none tabular-nums sm:mt-2 sm:text-[13px]">
            ...
            <span className="font-bold text-[#E65100]">{price ? formatRsAmount(price) : 'Enquire'}</span>
          </p>
        </div>

        {available || inCart ? (
          <div className="flex items-center justify-between gap-2 border-t border-slate-100 pt-2">
            <div className="inline-flex h-8 overflow-hidden rounded-lg border border-[#004D55]/15 bg-white shadow-sm">
              <button
                type="button"
                onClick={() => handleQuantityChange(quantity - 1)}
                disabled={!inCart}
                className="flex w-8 items-center justify-center border-r border-[#004D55]/10 text-[#004D55] transition hover:bg-[#004D55]/5 disabled:cursor-not-allowed disabled:text-slate-300"
                aria-label="Decrease quantity"
              >
                <Minus className="h-3.5 w-3.5" />
              </button>
              <span className="flex min-w-[2rem] items-center justify-center border-r border-[#004D55]/10 text-sm font-extrabold tabular-nums text-[#004D55]">
                {shownQty}
              </span>
              <button
                type="button"
                onClick={inCart ? () => handleQuantityChange(quantity + 1) : handleAddToCart}
                className="flex w-8 items-center justify-center bg-[#004D55] text-white transition hover:bg-[#006670]"
            ...
            <p className="text-sm font-extrabold tabular-nums text-[#004D55]">{lineTotal}</p>
          </div>
        ) : (
