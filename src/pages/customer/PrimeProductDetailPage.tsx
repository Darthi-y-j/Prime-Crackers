    ...
    return (
      <EmptyState
        ...
        action={
          <Link
            to="/#shop"
            className="rounded-full bg-[#004D55] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#006670]"
          >
            Back to shop
          </Link>
        }
    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-3 py-8 sm:px-6 sm:py-10">
        <nav className="mb-5 flex items-center gap-1.5 text-xs text-slate-500">
          <Link to="/" className="hover:text-[#004D55]">
            Home
          </Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <Link to="/#shop" className="hover:text-[#004D55]">
            Shop
          </Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="truncate font-semibold text-[#004D55]">{product.name}</span>
        </nav>
            ...
            <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {related.map((item) => (
                <li key={item.id}>
                  <ProductLink
                    product={item}
                    className="flex gap-3 rounded-xl border border-[#004D55]/15 bg-white p-3 shadow-sm hover:border-[#FFC107]"
                  >
                    <img
                      src={getImageUrl(item.image_url, '/placeholder-product.svg', IMAGE_WIDTH.thumb)}
                      alt=""
                      className="h-16 w-16 shrink-0 rounded-md border-2 border-[#FFC107] object-cover"
                    />
                    <div className="min-w-0">
                      <p className="truncate font-bold text-[#1A1A1A]">{item.name}</p>
                      <p className="mt-1 text-sm font-extrabold text-[#E65100]">
                        {formatPrice(resolveProductPrice(item)) ?? 'Enquire'}
                      </p>
                      <span className="mt-1 inline-flex items-center gap-1 text-[11px] font-bold text-[#004D55]">
...
function PrimeProductView({ product }: { product: Product }) {
  ...
  const specsPanel = specs.length > 0 ? (
    <div className="overflow-hidden rounded-lg border border-[#004D55]/15 lg:h-full">
      <div className="border-b border-[#FFC107]/25 bg-[#FFF8E1]/70 px-3 py-2">
        <h2 className="text-[11px] font-bold uppercase tracking-wide text-[#004D55]">
          Specifications
        </h2>
      </div>
  ...
  return (
    <div
      ...
      className={`grid gap-6 rounded-xl border-2 border-[#004D55]/15 bg-white p-4 shadow-md sm:p-6 ${
        specs.length > 0
    ...
    >
      <img
        ...
        alt={product.name}
        className="aspect-square w-full rounded-lg border-2 border-[#FFC107] object-cover"
      />

      <div className="min-w-0">
        <h1 className="font-display text-2xl font-extrabold text-[#004D55]">{product.name}</h1>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {hasDiscount && (
            <span className="inline-flex items-center rounded-md bg-[#E65100] px-2.5 py-1 text-xs font-extrabold tabular-nums tracking-wide text-white shadow-sm">
        ...
        <dl className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
          {brand && (
            <div>
              ...
              <dd>
                <span className="inline-block max-w-full truncate border-y-2 border-[#FFC107] px-1 py-0.5 text-sm font-bold text-[#004D55]">
                  {brand}
                </span>
              </dd>
            </div>
        ...
        <div className="mt-4">
          <span className="text-2xl font-extrabold text-[#E65100]">{price ?? 'Enquire'}</span>
          {originalPrice && (
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2">
          <WishlistButton
            product={product}
            className="h-11 w-11 rounded-md border border-[#004D55]/15 bg-white"
          />
          {!product.is_available && !inCart ? (
            ...
            <div className="inline-flex h-11 overflow-hidden rounded-md border border-[#004D55]/20 bg-white">
              <button
                type="button"
                onClick={() => handleQuantityChange(quantity - 1)}
                disabled={!inCart}
                className="flex w-11 items-center justify-center text-[#004D55] hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300"
                aria-label="Decrease quantity"
              >
                <Minus className="h-4 w-4" />
              </button>
              <span className="flex min-w-[2.5rem] items-center justify-center border-x border-[#004D55]/15 text-sm font-bold tabular-nums">
                {inCart ? quantity : 0}
              </span>
              <button
                type="button"
                onClick={inCart ? () => handleQuantityChange(quantity + 1) : handleAddToCart}
                className="flex min-w-11 items-center justify-center gap-1.5 bg-[#004D55] px-3 text-sm font-bold text-white hover:bg-[#006670]"
